...
import {
  branchFinancials, staffBillingInPeriod, staffIncentivesInPeriod, staffStatusForMonth,
...
export const OrgPulseScreen: React.FC = () => {
  ...
  const data = useMemo(() => {
    const perBranch = branches.map(b => ({
      b, ...branchFinancials(b, monthStr, entries, expenses, monthlyExpenses, staff, settings, leaves),
    }));
    const collection = perBranch.reduce((s, r) => s + r.revenue, 0);
    const net = perBranch.reduce((s, r) => s + r.net, 0);

    let incentive = 0;
    staff.forEach(st => { incentive += staffIncentivesInPeriod(st.id, entries, monthStr); });

    const activeStaff = staff.filter(st => staffStatusForMonth(st, monthStr).status !== 'inactive').length;

    const ranked = [...perBranch].sort((a, b) => b.net - a.net);

    const stylists = staff
      .map(st => ({ st, billing: staffBillingInPeriod(st.id, entries, monthStr) }))
      .filter(x => x.billing > 0)
      .sort((a, b) => b.billing - a.billing)
      .slice(0, 5);

    // MTD daily network collection
    const days = new Date(yr, mo, 0).getDate();
    const daily = Array.from({ length: days }, () => 0);
    entries.forEach(e => {
      if (!e.date || !e.date.startsWith(monthStr)) return;
      const d = Number(e.date.slice(8, 10));
      const matSale = (e.staff_billing || []).reduce((m: number, sb: any) => m + (Number(sb.material) || 0), 0);
      daily[d - 1] += (Number((e as any).online) || 0) + (Number(e.cash) || 0) + matSale;
    });
  ...
  }, [branches, staff, entries, expenses, monthlyExpenses, settings, leaves, monthStr, yr, mo]);
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: space.xl, gap: space.lg, paddingBottom: 90 }}>
      ...
      <View>
        <Text style={{ fontFamily: fonts.sansBold, fontSize: 10, letterSpacing: 1.8, textTransform: 'uppercase', color: colors.text3, marginBottom: 8 }}>Collection Trend · MTD</Text>
